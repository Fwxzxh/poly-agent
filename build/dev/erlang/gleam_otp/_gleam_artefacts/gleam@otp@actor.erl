...
-type message(FZL) :: {message, FZL} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(FZM, FZN) :: {continue,
        FZM,
        gleam@option:option(gleam@erlang@process:selector(FZN))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(FZO, FZP) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        FZO,
        gleam@erlang@process:selector(message(FZP)),
        gleam@otp@system:debug_state(),
        fun((FZO, FZP) -> next(FZO, FZP))}.

-type started(FZQ) :: {started, gleam@erlang@process:pid_(), FZQ}.

-opaque initialised(FZR, FZS, FZT) :: {initialised,
        FZR,
        gleam@option:option(gleam@erlang@process:selector(FZS)),
        FZT}.

-opaque builder(FZU, FZV, FZW) :: {builder,
        fun((gleam@erlang@process:subject(FZV)) -> {ok,
                initialised(FZU, FZV, FZW)} |
            {error, binary()}),
        integer(),
        fun((FZU, FZV) -> next(FZU, FZV)),
        gleam@option:option(gleam@erlang@process:name(FZV))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(FZX) :: {ack, {ok, FZX} | {error, binary()}} |
...
-spec continue(GAC) -> next(GAC, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(GAO, GAP), gleam@erlang@process:selector(GAP)) -> next(GAO, GAP).
...
-spec initialised(GAV) -> initialised(GAV, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(GBA, any(), GBC),
    gleam@erlang@process:selector(GBG)
) -> initialised(GBA, GBG, GBC).
...
-spec returning(initialised(GBL, GBM, any()), GBR) -> initialised(GBL, GBM, GBR).
...
-spec new(GBV) -> builder(GBV, GBW, gleam@erlang@process:subject(GBW)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(GCB)) -> {ok, initialised(GCD, GCB, GCE)} |
        {error, binary()})
) -> builder(GCD, GCB, GCE).
...
-spec on_message(builder(GCN, GCO, GCP), fun((GCN, GCO) -> next(GCN, GCO))) -> builder(GCN, GCO, GCP).
...
-spec named(builder(GCY, GCZ, GDA), gleam@erlang@process:name(GCZ)) -> builder(GCY, GCZ, GDA).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(GDN))) -> gleam@erlang@process:selector(message(GDN)).
...
-spec receive_message(self(any(), GDJ)) -> message(GDJ).
...
-spec initialise_actor(
    builder(any(), any(), GEG),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, GEG} | {error, binary()})
...
-spec start(builder(any(), any(), GET)) -> {ok, started(GET)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(GFA), GFA) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(GFC),
    integer(),
    fun((gleam@erlang@process:subject(GFE)) -> GFC)
) -> GFE.

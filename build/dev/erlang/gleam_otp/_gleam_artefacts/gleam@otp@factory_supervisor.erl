...
-opaque supervisor(GVE, GVF) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(GVE, GVF))}.

-type message(GVG, GVH) :: any() | {gleam_phantom, GVG, GVH}.

-opaque builder(GVI, GVJ) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((GVI) -> {ok, gleam@otp@actor:started(GVJ)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(GVI, GVJ)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(GVK, GVL) :: {local,
        gleam@erlang@process:name(message(GVK, GVL))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(GVM) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, GVM}.
...
-type erlang_child_spec_property(GVN, GVO) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((GVN) -> {ok, gleam@otp@actor:started(GVO)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(GVP, GVQ))) -> supervisor(GVP, GVQ).
get_by_name(Name) ->
...
-spec named(builder(GWG, GWH), gleam@erlang@process:name(message(GWG, GWH))) -> builder(GWG, GWH).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(GWP, GWQ), integer(), integer()) -> builder(GWP, GWQ).
...
-spec timeout(builder(GWV, GWW), integer()) -> builder(GWV, GWW).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(GXB, GXC), gleam@otp@supervision:restart()) -> builder(GXB, GXC).
...
-spec start(builder(GXH, GXI)) -> {ok,
        gleam@otp@actor:started(supervisor(GXH, GXI))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(GYG, GYH)) -> gleam@otp@supervision:child_specification(supervisor(GYG, GYH)).
supervised(Builder) ->
...
-spec start_child(supervisor(GYN, GYO), GYN) -> {ok,
        gleam@otp@actor:started(GYO)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((GZL) -> {ok, gleam@otp@actor:started(GZM)} |
        {error, gleam@otp@actor:start_error()}),
    GZL
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), GZM, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((GVW) -> {ok, gleam@otp@actor:started(GVX)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(GVW, GVX).
...
-spec supervisor_child(
    fun((GWB) -> {ok, gleam@otp@actor:started(GWC)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(GWB, GWC).

...
-type child_specification(GSQ) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(GSQ)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(GSR)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(GSR).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(GSW)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(GSW).
supervisor(Start) ->
...
-spec significant(child_specification(GTB), boolean()) -> child_specification(GTB).
...
-spec timeout(child_specification(GTE), integer()) -> child_specification(GTE).
timeout(Child, Ms) ->
...
-spec restart(child_specification(GTH), restart()) -> child_specification(GTH).
...
-spec map_data(child_specification(GTK), fun((GTK) -> GTM)) -> child_specification(GTM).

...
-spec be_ok({ok, HOB} | {error, any()}) -> HOB.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, HOG}) -> HOG.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(HOJ)) -> HOJ.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;

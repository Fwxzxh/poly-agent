...
-type response(FME) :: {response, integer(), list({binary(), binary()}), FME}.
...
-spec set_header(response(FMT), binary(), binary()) -> response(FMT).
...
-spec prepend_header(response(FMW), binary(), binary()) -> response(FMW).
...
-spec set_body(response(any()), FNB) -> response(FNB).
...
-spec try_map(response(FMF), fun((FMF) -> {ok, FMH} | {error, FMI})) -> {ok,
        response(FMH)} |
    {error, FMI}.
...
-spec map(response(FND), fun((FND) -> FNF)) -> response(FNF).
...
-spec expire_cookie(response(FNO), binary(), gleam@http@cookie:attributes()) -> response(FNO).

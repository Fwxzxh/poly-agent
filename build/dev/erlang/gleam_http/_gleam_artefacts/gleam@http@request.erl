...
-spec set_header(request(FCK), binary(), binary()) -> request(FCK).
...
-spec prepend_header(request(FCN), binary(), binary()) -> request(FCN).
...
-spec set_body(request(any()), FCS) -> request(FCS).
set_body(Req, Body) ->
    {request,
...
-spec map(request(FCU), fun((FCU) -> FCW)) -> request(FCW).
...
-spec set_query(request(FDG), list({binary(), binary()})) -> request(FDG).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(FDK), gleam@http:method()) -> request(FDK).
...
-spec set_scheme(request(FDR), gleam@http:scheme()) -> request(FDR).
...
-spec set_host(request(FDU), binary()) -> request(FDU).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(FDX), integer()) -> request(FDX).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(FEA), binary()) -> request(FEA).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(FED), binary(), binary()) -> request(FED).
...
-spec remove_cookie(request(FEJ), binary()) -> request(FEJ).

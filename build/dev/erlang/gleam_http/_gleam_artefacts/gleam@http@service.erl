...
-spec map_response_body(
    fun((FQA) -> gleam@http@response:response(FQB)),
    fun((FQB) -> FQD)
) -> fun((FQA) -> gleam@http@response:response(FQD)).
...
-spec prepend_response_header(
    fun((FQF) -> gleam@http@response:response(FQG)),
    binary(),
    binary()
) -> fun((FQF) -> gleam@http@response:response(FQG)).
...
-spec ensure_post(gleam@http@request:request(FQJ)) -> {ok,
        gleam@http@request:request(FQJ)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(FQS)) -> FQU)) -> fun((gleam@http@request:request(FQS)) -> FQU).

...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(FXD)) -> {ok, FXD} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(FWX)) -> {ok, FWX} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(FWT)) -> {ok, FWT} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(FXJ), fun((FXJ) -> json())) -> json().
...
-spec array(list(FXN), fun((FXN) -> json())) -> json().

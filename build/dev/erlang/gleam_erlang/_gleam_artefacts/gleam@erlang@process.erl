...
-opaque subject(EGI) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(EGI)}.

-type name(EGJ) :: any() | {gleam_phantom, EGJ}.

-type do_not_leak() :: any().

-type selector(EGK) :: any() | {gleam_phantom, EGK}.
...
-spec 'receive'(subject(EHI), integer()) -> {ok, EHI} | {error, nil}.
...
-spec receive_forever(subject(EHQ)) -> EHQ.
...
-spec selector_receive(selector(EHU), integer()) -> {ok, EHU} | {error, nil}.
...
-spec selector_receive_forever(selector(EHY)) -> EHY.
...
-spec map_selector(selector(EIA), fun((EIA) -> EIC)) -> selector(EIC).
map_selector(A, B) ->
...
-spec merge_selector(selector(EIE), selector(EIE)) -> selector(EIE).
merge_selector(A, B) ->
...
-spec select_map(selector(EIP), subject(EIR), fun((EIR) -> EIP)) -> selector(EIP).
...
-spec select(selector(EIL), subject(EIL)) -> selector(EIL).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> EIZ)
) -> selector(EIZ).
...
-spec select_other(selector(EJD), fun((gleam@dynamic:dynamic_()) -> EJD)) -> selector(EJD).
...
-spec deselect(selector(EIU), subject(any())) -> selector(EIU).
...
-spec select_specific_monitor(selector(EJP), monitor(), fun((down()) -> EJP)) -> selector(EJP).
...
-spec select_monitors(selector(EJS), fun((down()) -> EJS)) -> selector(EJS).
...
-spec select_trapped_exits(selector(EII), fun((exit_message()) -> EII)) -> selector(EII).
...
-spec deselect_specific_monitor(selector(EJV), monitor()) -> selector(EJV).
...
-spec send_after(subject(EKR), integer(), EKR) -> timer().
...
-spec perform_call(
    subject(EJY),
    fun((subject(EKA)) -> EJY),
    fun((selector(EKA)) -> {ok, EKA} | {error, nil})
) -> EKA.
...
-spec call(subject(EKF), integer(), fun((subject(EKH)) -> EKF)) -> EKH.
...
-spec call_forever(subject(EKJ), fun((subject(EKL)) -> EKJ)) -> EKL.
